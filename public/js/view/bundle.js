	...
	document
		...
		.addEventListener('click', async () => {
			try {
				...
				if (!name) {
					return (0,_util_toastify__WEBPACK_IMPORTED_MODULE_1__.toast)('danger', 'Vui lòng nhập tên');
				}
				const res = await (0,_util_fetchAPI__WEBPACK_IMPORTED_MODULE_0__.postDataAPI)('user/updateMe', { name });
				if (res.status === 200) {
					location.reload();
					(0,_util_toastify__WEBPACK_IMPORTED_MODULE_1__.toast)('success', 'Cập nhật thành công');
				}
			} catch (error) {
				(0,_util_toastify__WEBPACK_IMPORTED_MODULE_1__.toast)('danger', error.response.data.message);
			}
		...
		.addEventListener('click', async () => {
			try {
				...
				if (!passwordCurrent || !passwordConfirm || !password) {
					return (0,_util_toastify__WEBPACK_IMPORTED_MODULE_1__.toast)('danger', 'Vui lòng nhập tất cả các trường');
				}
